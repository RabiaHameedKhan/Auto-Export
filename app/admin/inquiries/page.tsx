  ...
  try {
    ...
    const list = await db
      .select({
        ...
        destinationPort: inquiries.destinationPort,
          ...
          <tbody>
            {rows.map((r) => (
              <tr key={r.id} className="border-b border-[#e0e0e0]">
                ...
                <td className="p-3">{r.destinationPort}</td>
                <td className="p-3">{r.phone}</td>
                <td className="p-3">{r.email}</td>
                <td className="max-w-[280px] p-3 whitespace-normal break-words">{r.address}</td>
                <td className="max-w-[280px] p-3 whitespace-normal break-words">{r.message}</td>
